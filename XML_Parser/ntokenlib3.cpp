...
int ClToken::getToken(ifstream &datei, ClElement *element, ClElement *wurzel) {
  int zaehler;
    ...
    case '>':
      ...
      if (zustand == istEndTag) {
        if (strcmp(tokenName, puffer)) {
          ...
          cout << "Erhalten: '" << puffer << "'; erwartet: '" << tokenName
               << "'" << endl;
          return fillToken(0);
        }
        return fillToken(1);
      ...
      if (zustand == istStartTag) {
        for (ziel = tokenName, quelle = puffer;
             *quelle != ' ' && *quelle != '\0'; ziel++, quelle++)
          *ziel = *quelle;
        *ziel = '\0';
        if (!strcmp(tokenName, wurzel->getName()))
          element = wurzel;
        else {
          if (element == NULL)
            isChild = -1;
          else
            isChild = element->elementIstErlaubt(tokenName);
          if (isChild < 0) {
...
void ClToken::druckeToken(int ebene) {
  ...
  if (tokenChild != NULL)
    tokenChild->druckeToken(ebene + 1);
  if (tokenSibling != NULL)
    tokenSibling->druckeToken(ebene);
}
